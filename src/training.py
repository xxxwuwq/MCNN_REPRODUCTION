...
def train():
    ...
    for i in range(cfig.total_iters):
        ...
        for file_index in range(len(img_file_list)):
            ...
            _, inf_count, loss = sess.run([optimizer, inference_density_map, joint_loss],
                                          feed_dict=feed_dict)
            format_time = str(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
            format_str = 'step %d, joint loss=%.5f, inference= %.5f, gt=%d'
            log_line = format_time, img_file_list[file_index], format_str % (i * len(img_file_list) + file_index, loss, inf_count.sum(), gt_count)
        ...
        if i % 50 == 0:
            ...
            for file_index in range(len(val_img_file_list)):
                ...
                feed_dict = {input_img_placeholder: (img - 127.5) / 128, density_map_placeholder: gt_dmp}
                _, inf_count, loss = sess.run([optimizer, inference_density_map, joint_loss], feed_dict=feed_dict)

                format_time = str(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
                format_str = 'step %d, joint loss=%.5f, inference= %.5f, gt=%d'
                absolute_error = absolute_error + np.abs(np.subtract(gt_count, inf_count.sum())).mean()
                square_error = square_error + np.power(np.subtract(gt_count, inf_count.sum()), 2).mean()
                log_line = format_time, val_img_file_list[file_index], format_str % (file_index, loss, inf_count.sum(), gt_count)
